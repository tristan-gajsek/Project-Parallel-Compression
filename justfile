...
compress algorithm world_size *args:
    @mpirun -np {{world_size}} cargo r -rq -- -a {{algorithm}} c {{args}}

decompress algorithm world_size:
    @mpirun -np {{world_size}} cargo r -rq -- -a {{algorithm}} d

compress-stats algorithm world_size *args:
    @mpirun -np {{world_size}} cargo r -rq -- -p -a {{algorithm}} c {{args}}

decompress-stats algorithm world_size:
    @mpirun -np {{world_size}} cargo r -rq -- -p -a {{algorithm}} d
